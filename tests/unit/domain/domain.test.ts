...
describe('WpmSpeed', () => {
  ...
  it('rejects WPM above maximum', () => {
    expect(() => WpmSpeed.create(1500, new Date(), 1)).toThrow(InvalidValueError);
  });
...
describe('User', () => {
  ...
  it('rejects invalid email', () => {
    expect(() => User.create('u-1', 'not-an-email', defaultWpm)).toThrow(InvalidValueError);
  });
...
describe('Book', () => {
  const chapters = [
    { number: 1, title: 'The Beginning', wordCount: 3000, mood: 'calm' as const },
  ...
  it('rejects book with no chapters', () => {
    expect(() => Book.create('b-1', 'Empty', 'Author', [])).toThrow(InvalidValueError);
  });
});
...
describe('ReadingTimeCalculator', () => {
  it('applies immersion buffer to raw calculation', () => {
    ...
    expect(duration.inMinutes).toBe(Math.ceil(expectedRaw * IMMERSION_BUFFER_FACTOR));
  });
...
describe('ReadingSession state machine', () => {
  const playlist = Playlist.create('p-1', 'spotify-abc', 'alpha-waves', 30, 'Focus Mix');
