...
function buildRepositories(): { userRepo: UserRepositoryPort; bookRepo: BookRepositoryPort } {
...
async function bootstrap(): Promise<void> {
  ...
  const prepareUseCase = new PrepareReadingSessionInteractor(
    userRepo, bookRepo, spotifyService, idGenerator,
  );
  ...
  app.log.info(`Server listening on ${HOST}:${PORT} [adapter: ${process.env.DYNAMO_ENDPOINT ? 'DynamoDB' : 'InMemory'}]`);
}
