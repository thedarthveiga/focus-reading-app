...
gh api \
  ...
  --field name="release branches" \
  --field target=branch \
  --field enforcement=active \
  --field 'conditions={"ref_name":{"include":["refs/heads/release/**"],"exclude":[]}}' \
  --field 'rules=[
    ...
    {"type":"required_status_checks","parameters":{"required_status_checks":[{"context":"Type-check, Lint & Format"},{"context":"Unit tests"},{"context":"Integration tests (in-memory)"}],"strict_required_status_checks_policy":true}},
    {"type":"non_fast_forward"},
    {"type":"deletion"}
  ]' 2>/dev/null \
  && echo "✓ release/* ruleset created." \
  || echo "  ⚠ Rulesets API requires GitHub Team/Enterprise — skipping release/* ruleset. Use branch-policy.yml enforcement instead."
